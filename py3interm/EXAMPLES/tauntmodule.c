...
gcc -c -I/usr/include/python3.2 tauntlibmodule.c
...
tauntlib_taunt(PyObject *self, PyObject *args) {
    ...
    return PyUnicode_FromString(taunts[taunt_index]);
}
...
tauntlib_tauntcount(PyObject *self,PyObject *args) {
    return PyLong_FromLong(number_of_taunts());
}

static PyMethodDef tauntmethods[] = {
...
static struct PyModuleDef tauntmodule =
{
...
PyInit_taunt(void) {
    ...
    t = PyModule_Create(&tauntmodule);
    if (t == NULL) {
        return NULL;
    }
    
    TauntError = PyErr_NewException("taunt.taunterror", NULL, NULL);
    Py_INCREF(TauntError);
    PyModule_AddObject(t, "taunterror", TauntError);
